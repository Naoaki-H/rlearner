#' @include utils.R
#'
#' @title R-learner, implemented via lightgbm (boosting)
#'
#' @description  R-learner, as proposed by Nie and Wager (2017), implemented via lightgbm (boosting)
#'
...
rlgb= function(x, w, y,
  ...
  if (is.null(m_hat)){
    ...
    m_hat = predict(object=y_fit$lgb_fit,x)
  }
  else {
    y_fit = NULL
  }
  ...
  if (is.null(p_hat)){
    w_fit = cvboost(x,
                    ...
                    nthread = nthread,
                    verbose = verbose)
    p_hat = predict(object=w_fit$lgb_fit,x)
  }
  else{
    w_fit = NULL
  }
...
predict.rboost<- function(object,
  ...
  if (tau_only) {
    predict(object$tau_fit, x = newx)
  } else {
    tau = predict(object$tau_fit, x = newx)
    e = predict(object$w_fit, x = newx)
    m = predict(object$y_fit, x = newx)
    mu1 = m + (1-e) * tau
    mu0 = m - e * tau
